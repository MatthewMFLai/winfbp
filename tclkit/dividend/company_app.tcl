...
proc process_template {filename p_keylist} {
	...
	set keylist [concat $keylist [Company::get_keylist]]
	set keylist [lsort -unique $keylist]
	return [Company::get_all_symbols] 
    }
	...
	set result [Company::search_db $key $lower $upper]
...
source company_db.tcl

set filename [lindex $argv 0]
Company::init $filename
set keylist "Symbol"
set symbollist [process_template [lindex $argv 1] keylist]
puts $symbollist

# Put description as the last column.
#set idx [lsearch $keylist "symbol"]
#set keylist [lreplace $keylist $idx $idx]
#set idx [lsearch $keylist "description"]
#set keylist [lreplace $keylist $idx $idx]
#lappend $keylist "description"
...
foreach symbol $symbollist {
    ...
    set dataline ""
    Company::get_symbol $symbol tmptable
    foreach key $keylist {
