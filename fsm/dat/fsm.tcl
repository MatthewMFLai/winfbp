...
proc set_init_proc {p_generic attr} {
    ...
    set generic(&init_proc) $attr
}

proc get_init_proc {p_generic} {
    upvar #0 $p_generic generic

    return $generic(&init_proc)
}

proc set_in_service {p_generic attr} {
    upvar #0 $p_generic generic

    set generic(&in_service) $attr
}

proc get_in_service {p_generic} {
    upvar #0 $p_generic generic

    return $generic(&in_service)
}

proc set_last_error {p_generic attr} {
    upvar #0 $p_generic generic

    set generic(&last_error) $attr
}

proc get_last_error {p_generic} {
    upvar #0 $p_generic generic

    return $generic(&last_error)
}
...
proc init {p_generic} {
    upvar #0 $p_generic generic
    set generic(&init_proc) ""
    set generic(&in_service) ""
    set generic(&last_error) ""
}

proc remove {p_generic} {
    upvar #0 $p_generic generic
    unset generic(&init_proc)
    unset generic(&in_service)
    unset generic(&last_error)
}
