...
while {$suffix <= $dimension} {
    ...
    puts $fd "    lappend vertex_$suffix\(&plane_list\) \$p_plane"
    incr suffix 
}
puts $fd "\}"
puts $fd ""
...
set suffix 1
while {$suffix <= $dimension} {
    puts $fd "    set data \$vertex_$suffix\(&plane_list\)"
    puts $fd "    set idx \[lsearch \$data \$p_plane\]"
    puts $fd "    if \{\$idx > -1\} \{"
    puts $fd "        set vertex_$suffix\(&plane_list\) \[lreplace \$data \$idx \$idx\]"
    puts $fd "    \} else \{"
    puts $fd "        return -2"
    puts $fd "    \}"
    puts $fd ""
...
set suffix 1
while {$suffix <= $dimension} {
    puts $fd "    set planelist$suffix \$vertex_$suffix\(&plane_list\)"
    incr suffix
}
puts $fd ""
...
puts $fd "proc get_plane_iterator \{p_vertex\} \{"
puts $fd "    upvar #0 \$p_vertex vertex"
puts $fd ""
puts $fd "    return \$vertex\(&plane_list\)"
puts $fd "\}"
puts $fd ""
